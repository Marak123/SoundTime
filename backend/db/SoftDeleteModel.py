"""
    ...
    File source:   https://github.com/exploreshaifali/django-softdelete-it
    License:       MIT License
    Author Name:   Shaifali Agrawal
    Author email:  agrawalshaifali09@gmail.com


    Note:
        The file has been modified accordingly, in accordance with the licence
...
class SoftDeleteHelper:
            ...
            try:
                if model._meta.get_field("deleted"):
                    collector.data[model] = set(
                        filter(lambda x: not x.deleted, instances)
                    )
    ...
    def sql_model_wise_batch_update(self, model, instances, deleted=None):
        ...
        query.update_batch(
            [obj.pk for obj in instances], {"deleted": deleted}, self.using
        )
    ...
    @transaction.atomic
    def do_work(self, objs, deleted_uuid=None):
        """
            ...
            try:
                if self.delete_type == "soft_delete":
                    self.sql_model_wise_batch_update(
                        model, instances, deleted=(deleted_uuid or uuid.uuid4())
                    )
                else:
...
class SoftDeleteQuerySet(models.QuerySet):
    @transaction.atomic
    def delete(self, using=None, deleted_uuid=None):
        ...
        return helper.do_work(self, deleted_uuid)
